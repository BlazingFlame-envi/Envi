...
class PartenaireController extends AbstractController
{
    ...
    public function fnew(Request $request , EntityManagerInterface $entityManagerInterface)
    {
     ...
     $author = new Entreprise();
       ...
       return $this->render('partenaire/ajouterEntreprise.html.twig',['f'=> $form->createView(),]);
        ...
        return $this->redirectToRoute('app_partenaire'); // Redirection vers une page d'accueil ou une autre page appropriée
    }
    ...
    public function delete($id ,EntityManagerInterface $entityManagerInterface , EntrepriseRepository $authorRepository)
    {
       ...
       dd($author);
            ...
            // Si le rôle de l'utilisateur n'est pas 'agence', rediriger ou afficher un message d'erreur
            $this->addFlash('error', 'You do not have permission to access this page.');
            return $this->redirectToRoute('app_partenaire_list'); // Redirection vers une page d'accueil ou une autre page appropriée
        }
       
    }

    #[Route('/listpart', name: 'app_partenaire_list')]
    public function index7(EntityManagerInterface $entityManagerInterface,EntrepriseRepository $authorRepository): Response
    {
        
        $user = $this->getUser();
        $role = $user->getRole();
        if ($role === 'agence') {
       $listEntreprise = $entityManagerInterface->getRepository(Entreprise::class)->getEntrepriseByUser($user);
       return $this->render('partenaire/listpart.html.twig', [
        'controller_name' => 'AuthorController',
        'partenaires' => $listEntreprise, 
    ]);
}
